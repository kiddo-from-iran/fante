/// Gregorian → Jalali (Persian) date helpers for display.
class JalaliDate {
  JalaliDate._();

  /// Formats as `yyyy/MM/dd` in Jalali calendar.
  static String format(DateTime date) {
    final j = toJalali(date.year, date.month, date.day);
    final y = j.$1;
    final m = j.$2.toString().padLeft(2, '0');
    final d = j.$3.toString().padLeft(2, '0');
    return '$y/$m/$d';
  }

  /// Returns `(year, month, day)` in Jalali.
  static (int, int, int) toJalali(int gy, int gm, int gd) {
    final gdm = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334];
    var gy2 = (gm > 2) ? (gy + 1) : gy;
    var days = 355666 +
        (365 * gy) +
        ((gy2 + 3) ~/ 4) -
        ((gy2 + 99) ~/ 100) +
        ((gy2 + 399) ~/ 400) +
        gd +
        gdm[gm - 1];
    var jy = -1595 + (33 * (days ~/ 12053));
    days %= 12053;
    jy += 4 * (days ~/ 1461);
    days %= 1461;
    if (days > 365) {
      jy += (days - 1) ~/ 365;
      days = (days - 1) % 365;
    }
    final jm = (days < 186) ? 1 + (days ~/ 31) : 7 + ((days - 186) ~/ 30);
    final jd = 1 + ((days < 186) ? (days % 31) : ((days - 186) % 30));
    return (jy, jm, jd);
  }
}
